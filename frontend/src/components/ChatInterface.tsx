...
const ChatInterface = ({ documents }: ChatInterfaceProps) => {
            ...
            if (trimmedLine.startsWith('* **') && trimmedLine.includes('**:')) {
              ...
              return (
                <div key={index} className="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-4 rounded-r-lg shadow-sm">
                  ...
                  <p className="text-blue-800 leading-relaxed ml-5">{definition}</p>
                </div>
              )
              ...
              return (
                <div key={index} className="ml-6 border-l-2 border-slate-300 pl-4 py-3 bg-slate-50 rounded-r-lg">
                  ...
                  <p className="text-slate-700 text-sm leading-relaxed ml-3.5">{definition}</p>
                </div>
              )
  ...
  return (
    <div className="max-w-5xl mx-auto">
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${
                queryType === type
                  ...
                  : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 hover:shadow-md'
              }`}
            >
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center space-x-3">
          <select
            ...
            className="flex-1 px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white shadow-sm"
          >
          ...
          <button
            ...
            className="px-4 py-3 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 transition-colors shadow-sm"
      ...
      {/* Chat Messages */}
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/50 p-6 mb-8 h-96 overflow-y-auto shadow-lg">
        <div className="space-y-6">
          {messages.map((message) => (
            ...
            >
              <div
                className={`max-w-xs lg:max-w-lg px-6 py-4 rounded-2xl shadow-sm ${
                  message.type === 'user'
                ...
                {message.sources && message.sources.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-200">
                    ...
                    {message.sources.map((source, index) => (
                      <p key={index} className="text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded mb-1">{source}</p>
                    ))}
                  </div>
                )}
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-slate-50 text-slate-900 px-6 py-4 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex items-center space-x-3">
      ...
      <div className="flex space-x-4">
        <div className="flex-1">
          <textarea
            ...
            className="w-full px-6 py-4 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none bg-white/80 backdrop-blur-sm shadow-sm"
            rows={3}
          />
        </div>
        <button
          ...
          className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105"
        >
      ...
      {/* Quick Actions */}
      <div className="mt-12">
        <h3 className="text-xl font-semibold text-slate-900 mb-6 text-center">Quick Actions</h3>
        <div className="grid md:grid-cols-3 gap-6">
          <button
            ...
            className="p-6 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl hover:border-blue-300 hover:shadow-lg transition-all duration-200 text-left group"
          >
            ...
            <h4 className="font-semibold text-slate-900 mb-2">Define Legal Terms</h4>
            <p className="text-sm text-slate-600">Get explanations of legal terminology</p>
          </button>
          ...
          >
            ...
            <h4 className="font-semibold text-slate-900 mb-2">Document Summary</h4>
            <p className="text-sm text-slate-600">Get summaries of specific topics</p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-slate-600">Get answers to specific questions</p>
          </button>
        </div>
      </div>
      ...
      {/* Document Status */}
      {documents.length > 0 && (
        <div className="mt-12 p-6 bg-slate-50/80 backdrop-blur-sm rounded-2xl border border-slate-200">
          ...
          <div className="flex flex-wrap gap-3">
            {documents.map((doc, index) => (
              <span
                key={index}
                className="px-4 py-2 bg-white text-slate-700 text-sm rounded-xl border border-slate-200 shadow-sm"
              >
                {doc}
              </span>
            ))}
      ...
      <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200">
        ...
        <p className="text-sm text-blue-700">
          ✅ Backend connected and running on <code className="bg-blue-100 px-2 py-1 rounded-lg font-mono">http://localhost:8001</code>
        </p>
